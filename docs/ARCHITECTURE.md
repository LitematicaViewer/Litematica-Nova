...
- `ui/windows/main/pages/ui-test/UiTestPage.tsx` 是当前项目各主题 UI 样式的可视标准页。新增或修改 UI 时，默认以该页展示的控件、间距、标题栏和子窗口行为作为标准；除非需求明确要求偏离，否则不要另起一套样式规则。
- 独立式子窗口与遮罩式子窗口必须共用同一套标题、副标题和内容区结构；样式层面唯一允许的默认差异是遮罩式多一个右上角关闭按钮。
- 子窗口标题样式默认复用主窗口页面标题设计，不再单独发明一套“子窗口专用标题样式”。
- 遮罩式子窗口默认采用与 UI 测试页示例一致的关闭按钮行为：关闭按钮位于右上角，正文内容区与独立式保持一致；不要为了遮罩式再拆出不同的标题栏语义结构。
