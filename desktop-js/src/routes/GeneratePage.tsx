...
import { getWorkspaceRoot, startNativeViewer, writeWorkspaceFile } from "../services/backend";
...
async function buildDefaultOutputPath(currentFile: string | undefined, projectionName: string): Promise<string> {
...
export function GeneratePage({ currentFile, setCurrentFile, setRoute }: any) {
  ...
  const handleExportTemplate = async () => {
    const selected = await saveDialog({
      defaultPath: `${sanitizeFileName(form.metadata.name || "custom_template")}.json`,
  ...
  const handleDryRun = async () => {
    ...
    try {
      ...
      setOutputPath(outputPath || await buildDefaultOutputPath(currentFile, form.metadata.name));
  ...
  const chooseOutput = async () => {
    const defaultPath = outputPath || await buildDefaultOutputPath(currentFile, form.metadata.name);
    const selected = await saveDialog({
      defaultPath,
  ...
  const savePlan = async () => {
    const selected = await saveDialog({
      defaultPath: "projection_plan.json",
      filters: [{ name: "JSON", extensions: ["json"] }],
    });
  ...
  const openFolder = async () => {
    ...
    if (index > 0) await openShell(generatedPath.slice(0, index));
  };
          ...
          <div className="group-box">
            ...
            {generatedPath && (
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
                ...
                <button className="btn" onClick={() => startNativeViewer(generatedPath, "full")}>直接打开 3D 预览</button>
