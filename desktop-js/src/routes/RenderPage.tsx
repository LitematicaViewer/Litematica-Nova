...
export function RenderPage({ currentFile }: any) {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      if (cancelled) return;
      const element = previewHostRef.current;
      if (!currentFile || !cacheReady || !element) {
      ...
      try {
        ...
        const status = await startEmbeddedViewer(currentFile, buildMode, rectPhysical, "render_interactive");
        if (cancelled) return;
  ...
  }, [currentFile, buildMode, cacheReady]);
  ...
  const buildPreview = async (file: string, mode: string) => {
    try {
      ...
      setStageText(`3D cache 已完成；preview png 生成失败：${err}`);
    }
  };
          ...
          if (nextProgress.ready) {
            ...
            if (precacheLayers) {
              try {
                ...
                finalStage = `3D cache 已完成；分层索引检查失败：${err}`;
              }
            }
  ...
  const handleOpenPopup = async () => {
    if (!currentFile || !cacheReady) return;
    await startNativeViewer(currentFile, buildMode);
  };
  ...
  const setBuildMode = (value: string) => {
    setBuildModeState(saveDisplayMode(value));
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: 12 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <button className="btn" onClick={handleOpenPopup} disabled={!currentFile || !cacheReady}>打开弹窗 Viewer</button>
