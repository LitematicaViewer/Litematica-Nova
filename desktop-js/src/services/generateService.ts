...
async function writeTempPlan(plan: ProjectionPlan): Promise<{ planPath: string; planInfo: PathInfo }> {
  const planPath = `.tmp/desktop-js/generate/projection_plan_${Date.now()}_${Math.random().toString(16).slice(2)}.json`;
  try {
    await writeWorkspaceFile(planPath, JSON.stringify(plan, null, 2));
  } catch (error: any) {
