    ...
    return (
        <section className="page-pad options-page">
            ...
            <fieldset>
                ...
                <label className="setting-row"><input type="checkbox" /> 显示控件信息（已弃用，现在按 F12 可查看控件信息）</label>
                <label className="setting-row"><input type="checkbox" /> 性能测试（洋红圆动画 + 左下角 FPS 浮层）</label>
            </fieldset>
            <fieldset>
                <legend>性能与预加载</legend>
                <FormRow label="方块图标预加载时机：">
                    <select>
                        <option>软件启动时（默认）</option>
                        <option>首次加载投影文件时</option>
                        <option>首次点击材料列表或分层时</option>
                        <option>不加载方块图标（回退手段）</option>
                    </select>
                </FormRow>
                <FormRow label="材料列表计算时机：">
                    <select>
                        <option>首次加载投影文件时（默认）</option>
                        <option>首次点击材料列表时</option>
                    </select>
                </FormRow>
                <FormRow label="材料列表扫描方案：">
                    <select>
                        <option>Native Bridge 扫描（高性能，仅整投影）</option>
                        <option>Python 扫描（支持子区域）</option>
                    </select>
                </FormRow>
            </fieldset>
            <fieldset>
                ...
                <p className="muted">方块图标将被用于“材料列表”和“分层”。</p>
                <button className="wide-button" type="button">管理物品图标...</button>
                <p className="muted">物品图标将被用于“材料列表”和“分层”。</p>
            </fieldset>
        </section>
    );
}
