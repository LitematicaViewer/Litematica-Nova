    ...
    if (Array.isArray(parsed)) {
      ...
      parsed.forEach((state) => {
        if (state?.currentFile && state?.displayMode) {
          const restored: RenderCacheState =
            state.status === "building"
              ? {
                  ...state,
                  status: "error",
                  progress: null,
                  stage: "上次 3D cache 构建没有正常结束，已解锁；请重新构建。",
                  updatedAt: Date.now(),
                }
              : state;
          changed = changed || restored !== state;
          states.set(keyFor(restored.currentFile, restored.displayMode), restored);
        }
      });
      if (changed) save();
    }
