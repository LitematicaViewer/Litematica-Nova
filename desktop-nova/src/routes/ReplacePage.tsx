...
import { executeBackend, getUserConfigFilePath, writeUserConfigFile, checkFileExists } from '../services/backend';
...
export function ReplacePage({ currentFile }: any) {
  ...
  const handleDryRun = async () => {
    ...
    try {
      const res = await executeBackend("litematica_core.exe", ["replace-blocks", "--input", currentFile, "--rules", rulesPath, "--dry-run"]);
      setDryRunRes(res);
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const res = await executeBackend("litematica_core.exe", ["replace-blocks", "--input", currentFile, "--output", outPath, "--rules", rulesPath]);
      alert("替换完成！\n文件已保存至:\n" + outPath);
      setLog(log + "\n" + res);
    } catch(e: any) {
      alert("后端返回错误代码: " + e);
